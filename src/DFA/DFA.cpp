...
namespace DFA {
// DFA::build() lives in DFA_build.cpp (TDFA construction).
void DFA::optimizeRegistersAndLRTable() {
  ...
  auto dedup_hash = [](const NFA::ActionState &entry) -> std::size_t {
    ...
    combine(hash, uhash{}(entry.operand_a));
    combine(hash, uhash{}(entry.operand_b));
    return hash;
  };
  ...
  auto canonicalize = [&](auto self, std::size_t idx) -> std::size_t {
    ...
    for (std::size_t j : bucket) {
      if (dedup_table[j].action == entry.action &&
          ...
          dedup_table[j].operand_a == entry.operand_a &&
          dedup_table[j].operand_b == entry.operand_b &&
...
auto DFA::minimize() -> States<State<>> {
  ...
  auto hash_raw_action = [&](const NFA::ActionState &action,
    ...
    // TDFA register operands
    hash_combine(hash, uhash{}(action.operand_a));
    hash_combine(hash, uhash{}(action.operand_b));
                ...
                std::visit(
                    [&](const auto &action) {
                      ...
                      if constexpr (std::is_same_v<A, NFA::ActionState>) {
                        ...
                        action_identity.push_back(uhash{}(action.operand_a));

                        action_identity.push_back(uhash{}(action.operand_b));
                      } else {
                        // Type marker.
